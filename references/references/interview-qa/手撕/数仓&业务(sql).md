...
# 数据中台 / 数仓 专属「手撕 SQL 面试题库」

全是**面试现场手写必考题型**，包含 MySQL \+ Hive 高频 SQL，每题给**表结构 \+ 业务需求 \+ 标准参考答案**，直接背就能面试默写。
...
user\_id（用户 ID）、user\_name、sex、age、register\_time

### 2. 订单表 `order_info`

order\_id、user\_id、order\_amount、pay\_time、order\_status（0 未支付 1 已支付 2 已取消）

### 3. 行为日志表 `user_behavior`

id、user\_id、event\_type（浏览 / 加购 / 下单 / 支付）、create\_time

---

## 题 1：查询每个年龄段用户人数，按年龄段分组

**需求**：按 18 岁以下、18\-25、26\-35、35 以上 统计人数
...
## 题 3：统计每日下单用户数、支付用户数、下单金额

```sql
SELECT
    DATE(pay_time) AS dt,
    COUNT(DISTINCT user_id) AS order_user_cnt,
    COUNT(DISTINCT CASE WHEN order_status=1 THEN user_id END) AS pay_user_cnt,
    SUM(CASE WHEN order_status=1 THEN order_amount ELSE 0 END) AS total_pay_amount
FROM order_info
...
-- 身份证脱敏 110101********1234
SELECT CONCAT(SUBSTR(id_card,1,6),'********',SUBSTR(id_card,15,4)) AS id_card_mask
```

## 题 6：删除重复数据，保留 id 最小的一条
...
## 题 2：窗口函数排名：每日下单金额 TOP3 用户

同分不并列用 `row_number`，并列用 `rank`
...
SELECT * FROM (
    SELECT
        ...
        RANK() OVER(PARTITION BY dt ORDER BY SUM(order_amount) DESC) AS rk
...
SELECT
    ...
    ROUND(COUNT(DISTINCT ub.user_id)/COUNT(DISTINCT uf.user_id),4) AS retain_rate
FROM user_first uf
...
## 题 5：Hive 去重标准写法（面试最爱）

```sql
INSERT OVERWRITE TABLE dwd_user_behavior_d PARTITION(dt='2026-05-05')
SELECT
    user_id,event_type,create_time
FROM (
    SELECT
        user_id,event_type,create_time,
        ROW_NUMBER() OVER(PARTITION BY user_id,event_type ORDER BY create_time DESC) AS rn
...
## 题 6：拉链表 SCD2 核心手撕 SQL（用户维度历史回溯）
...
SELECT
    ...
    start_date,DATE_ADD('2026-05-05',-1) AS end_date,
    0 AS is_current
FROM dim_user_zip
WHERE is_current=1
...
## 题 7：Hive 数据倾斜 加盐打散 标准手撕 SQL

**场景：group by 某热门 key 倾斜**
...
## 题 8：列转行（炸裂函数 lateral view explode）

```sql
-- 假设标签字段 tag_list：'购物,娱乐,财经'
SELECT
    user_id,
    tag
FROM dwd_user_tag_d
...
**适用场景**：面试现场手写代码、纯业务实战、无理论题
**核心考点**：CTE、窗口函数、多表关联、业务指标计算、空值处理、分组聚合
**语法兼容**：MySQL 8.0+Hive（通用数仓语法）
**业务场景**：工厂生产、订单、库存全链路业务
